...
function ReasonItem({reason, setReason, reasonImg, setReasonImg, reasonText, setReasonText}){
    ...
    return(
        <div>
            ...
            <input className="bg-transparent w-[85px] text-center"
            value={reason}
        ...
        <div className={`bg-[#D9AB91] w-[40vw] md:w-[185px] md:min-h-[225px] rounded-md border-redFont border-solid border-2 ${modal ? 'block' : 'hidden'} mx-1 md:mx-4 mt-2`}>
            <div className="flex flex-col items-center justify-evenly h-full px-1 py-2 md:px-3" >
                <div className="cursor-pointer" {...getRootProps()}>
                    <div className={`flex items-center md:my-2 justify-center w-[80px] h-[80px] rounded-full bg-cover border-2 border-redFont ${backgroundImg ? 'border-none text-transparent' : ''}`} 
                    style={{backgroundImage: backgroundImg ? `url(${backgroundImg})` : 'none'}}>
                ...
                <div>
                    <textarea type={'text'}
                    className={`bg-transparent text-center text-redFont font-bold w-[100%] h-[100px] `}
                    value={reasonText}
